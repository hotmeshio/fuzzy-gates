...
import { ExportFormat, PruneTarget, RawTask, TaskConfig, TaskExport } from "../../../types/task";
...
/**
 * Finds the target task by ID, assembles the ancestor task list,
 * and identifies prior and following sibling tasks.

 */
export const findTaskDetails = (tasks: RawTask[], targetId: string): {ancestorTasks: string[], targetSiblings: string[], parentSiblings: string[], targetTask: RawTask, priorSibling: string, followingSibling: string, priorUncle: string | undefined, followingUncle: string | undefined} => {
  ...
  let priorUncle: string | undefined;
  let followingUncle: string | undefined;
  const parentSiblings: string[] = [];
  const targetSiblings: string[] = [];
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    if (currentDepth.length < targetDepthLength && targetDepth.slice(0, currentDepth.length).join('.') === currentDepth.join('.')) {
      ...
      //check if the current task is the prior sibling of the parent task
      if (currentDepth.length === targetDepthLength - 1) {
        parentSiblings.push(currentTask._task);
        ...
        if (currentDepthLast === targetDepthLast - 1) {
          priorUncle = currentTask._task;
        }
        if (currentDepthLast === targetDepthLast + 1) {
          followingUncle = currentTask._task;
        }
      }
    }
    ...
    if (
      ...
      targetSiblings.push(currentTask == targetTask ? '${CURRENT TASK INSTRUCTIONS}' : currentTask._task);
      const currentDepthLast = parseInt(currentDepth[currentDepth.length - 1], 10);
      const targetDepthLast = parseInt(targetDepth[targetDepth.length - 1], 10);
 ...
 */
export const findTasks = async (originId: string, config: TaskConfig, pruneTarget: PruneTarget = 'y'): Promise<RawTask[]> => {
  const task = MeshOS.findEntity(config.database, config.namespace, 'task');
 ...
 */
export const findOrigin = async (originId: string, config: TaskConfig): Promise<RawTask> => {
  const task = MeshOS.findEntity(config.database, config.namespace, 'task');
  const results = await task.meshData.find(
    'task',
    { namespace: config.namespace },
...
export const exportTasks = async (id: string, format: ExportFormat, config: { namespace: string, database: string }): Promise<string> => {
  ...
  tasks.forEach((task, index) => {
    output.push(renderTaskAsMarkdown(task));
  });
  return output.join('');
}
...
/**
 * Cleans up a task string by removing leading numbers and ensuring it ends with a period.
 */
export const cleanTask = (input: string): string => {
  const withoutNumber = input.replace(/^\d+\.\s*/, '');
  const endsWithPeriod = withoutNumber.trim().endsWith('.');
  return endsWithPeriod ? withoutNumber : withoutNumber.trim() + '.';
};
