...
import { normalizeWhitespace } from '../modules/utils';
import { ClarifyResponse, ExpansionResponse, GestaltResponse, PruneResponse, TaskOutput } from '../types/task';
...
class GPTService {
  ...
  async evaluateGestalt(messages: ChatMessage[], config?: { model?: string }): Promise<GestaltResponse | PruneResponse | ClarifyResponse | ExpansionResponse | APIErrorResponse> {
    ...
    const response = await openai.chat.completions.create({
      ...
      temperature: 0.1,
      max_tokens: 4096,
      top_p: 0.8, // Use nucleus sampling for balance between creativity and relevance
  ...
  async generateTaskTree(messages: ChatMessage[]): Promise<TaskOutput | APIErrorResponse> {
    const now = Date.now();
    const response = await openai.chat.completions.create({
